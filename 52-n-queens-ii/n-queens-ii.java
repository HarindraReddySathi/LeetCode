class Solution {
    public int totalNQueens(int n) {
        ...
        return queens(ans,up,down,x,y,temp,0,n);
        //return ans;
    }
    public int queens(List<List<String>> ans, boolean[] up, boolean[] down, boolean[] x, boolean[] y,
            int[][] temp, int level,int n) {
                ...
                for(int j=0;j<n;j++){
                    if(!up[level+j] && !down[n-level+j] && !x[level] && !y[j] ){
                        ...
                        res+=queens(ans,up,down,x,y,temp,level+1,n);
